...
export default function JellyHeart() {
    const containerRef = useRef<HTMLDivElement>(null);

    // Refs
    const characterGroupRef = useRef<SVGGElement>(null);
    const leftPupilRef = useRef<SVGCircleElement>(null);
    const rightPupilRef = useRef<SVGCircleElement>(null);

    useGSAP(() => {
        if (!containerRef.current || !characterGroupRef.current) return;

        // 1. Ruch źrenic - QuickTo dla płynności
        const pupilLeftX = gsap.quickTo(leftPupilRef.current, "x", { duration: 0.25, ease: "power2.out" });
        const pupilLeftY = gsap.quickTo(leftPupilRef.current, "y", { duration: 0.25, ease: "power2.out" });
        const pupilRightX = gsap.quickTo(rightPupilRef.current, "x", { duration: 0.25, ease: "power2.out" });
        const pupilRightY = gsap.quickTo(rightPupilRef.current, "y", { duration: 0.25, ease: "power2.out" });

        // 2. Idle Animation - Bardzo subtelne oddychanie
        gsap.to(characterGroupRef.current, {
            scale: 1.015, // Jeszcze delikatniejszy ruch
            duration: 2.5,
            ease: "sine.inOut",
            yoyo: true,
            repeat: -1,
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            // ZASIĘG RUCHU OCZU (zmniejszony dla realizmu)
            // Wcześniej 8px, teraz 5px - eliminuje efekt "uciekania" oczu
            const maxEyeMovement = 5;

            // Normalizacja (zakres -1 do 1)
            const xNorm = mouseX / (window.innerWidth / 2);
            const yNorm = mouseY / (window.innerHeight / 2);

            const xClamped = Math.max(-1, Math.min(1, xNorm));
            const yClamped = Math.max(-1, Math.min(1, yNorm));

            // Aplikacja ruchu źrenic
            pupilLeftX(xClamped * maxEyeMovement);
            pupilLeftY(yClamped * maxEyeMovement);
            pupilRightX(xClamped * maxEyeMovement);
            pupilRightY(yClamped * maxEyeMovement);
        };
        ...
        >
            ...
            >
                <g ref={characterGroupRef} className="origin-center">
                    ...
                    {/* 3. OCZY - Skorygowane pozycje */}

                    {/* Prawe Oko (w kodzie jako pierwsze od prawej w SVG) */}
                    <g>
                        <path
                            d="m323.215,180.532c-11.437,0.54-21.869-7.32-24.496-19.698-2.074-9.769 0.643-20.268 14.992-27.034 16.595-7.825 38.338,2.266 41.542,19.456 1.241,6.661-1.597,12.154-5.984,16.317-6.883,6.532-14.978,11.249-26.054,10.959z"
                            className="fill-white"
                        />

                        <circle ref={rightPupilRef} cx="325" cy="158" r="7" className="fill-primary-950" />
                    </g>


                    <g>
                        <path
                            d="m138.898,167.131c10.239,0.462 18.598,4.189 24.964,12.702 8.553,11.437 4.653,30.666-13.57,34.835-11.712,2.68-22.732-0.233-30.92-9.83-6.064-7.108-7.682-15.335-4.207-24.161 3.714-9.432 12.03-12.042 20.963-13.526 0.9-0.15 1.846-0.02 2.77-0.02z"
                            className="fill-white"
                        />

                        <circle ref={leftPupilRef} cx="140" cy="192" r="7" className="fill-primary-950" />
                    </g>

                    {/* 4. USTA */}
                    <path
                        d="m233.191,201.992c-18.463-0.352-33.732-13.583-33.995-29.657-0.069-4.211 0.406-8.07 5.452-8.782 4.457-0.629 6.549,2.393 7.9,6.115 2.375,6.541 5.583,12.351 12.453,15.308 7.538,3.245 14.041,1.509 20.36-3.255 6.251-4.712 6.39-11.098 4.713-17.471-1.391-5.288 0.388-8.403 4.769-10.206 4.253-1.75 9.037,1.207 10.836,6.156 7.199,19.813-5.703,34.627-22.764,40.173-3.09,1.006-6.475,1.103-9.724,1.619z"
                        className="fill-primary-800"
                    />

                    {/* Cieniowanie */}
                    <g className="fill-primary-600/20">
